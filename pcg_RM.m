function [x,flag,relres,iter,resvec] = pcg_RM(A,b,tol,maxit,K_inv,varargin)
...
N3 = length(b)/9;
n2b = norm(ddot42_2( K_inv, b, N3 ));                     % Norm of rhs vector, b
...
normr = norm(ddot42_2( K_inv, r, N3 ));                   % Norm of residual
normr_act = normr;

if (normr <= tolb || normr <= tol) % Initial guess is a good enough solution
    flag = 0;
    relres = normr / n2b;
    iter = 0;
    resvec = normr;
...
for ii = 1 : maxit
    ...
    r = r - alpha * q;
    normr = norm(ddot42_2( K_inv, r, N3 ));
    normr_act = normr;
    resvec(ii+1,1) = normr;
    
    % check for convergence
    if (normr <= tolb || normr <= tol || stag >= maxstagsteps || moresteps)
        r = b - iterapp('mtimes',afun,atype,afcnstr,x,varargin{:});
        normr_act = norm(ddot42_2( K_inv, r, N3 ));
        resvec(ii+1,1) = normr_act;
        if (normr_act <= tolb || normr <= tol)
            flag = 0;
            iter = ii;
            break
        else
...
if (flag == 0)
    ...
    normr_comp = norm(ddot42_2( K_inv, r_comp, N3 ));
    if normr_comp <= normr_act
        x = xmin;
        iter = imin;
        relres = normr_comp / n2b;
    else
        iter = ii;
        relres = normr_act / n2b;
    end
